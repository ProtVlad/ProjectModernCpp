...
std::vector<Word> WordStorage::GetWords()
{
	return m_db.get_all<Word>();
}

void WordStorage::AddWordToChosenWords(int id, int numberWords)
{
	ChosenWords w;
	w.SetId(id);
...
std::vector<std::tuple<std::string, int, int>> WordStorage::GetChosenWords()
{
	return m_db.select(
...
void WordStorage::PopulateListWords()
{
...
crow::response AddUserHandler::operator()(const crow::request& req) const
{
	...
	if (usernameIter != end && passwordIter != end)
	{
		User user;
		user.SetId(-1);
		user.SetUsername(usernameIter->second);
		user.SetPassword(passwordIter->second);
		m_db.insert(user);
	}
	return crow::response(201);
}
...
AddToChosenWordsHandler::AddToChosenWordsHandler(WordStorage& storage) :
	m_db{ storage }
{
}
...
crow::response AddToChosenWordsHandler::operator()(const crow::request& req) const
{
	auto bodyArgs = parseUrlArgs(req.body);
	auto end = bodyArgs.end();
	auto productIdIter = bodyArgs.find("id");
	auto quantityIter = bodyArgs.find("numberWords");
	if (productIdIter != end && quantityIter != end)
	{
